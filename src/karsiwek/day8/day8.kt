package karsiwek.day8

val input: String = """sdj dec 558 if r <= 8
...
dtf inc 304 if nwg > -1974""".trimIndent()
...
fun parseInstructions(input: String) {
    for(line in input.split("\n")){
        ...
        val register: String = elements[0];
        val operation: String = elements[1];
        val value: Int = Integer.parseInt(elements[2]);

        val conditionRegister: String = elements[4];
        val conditionOperator: String = elements[5];
        val conditionValue: Int = Integer.parseInt(elements[6]);

        if(checkCondition(conditionRegister, conditionOperator, conditionValue)){
            var registerValue : Int = getRegisterValue(register);
            registers[register] = if (operation=="inc") registerValue + value else registerValue - value;
            highestEver = Math.max(highestEver, registers[register]!!);
        }
    }

    System.out.println(registers.map { it.value }.max());
...
fun checkCondition(register: String, operator: String, value: Int) :Boolean {
    var registerValue : Int = getRegisterValue(register);
    when(operator){
