package karsiwek.day7

// NOT PROUD OF IT :(


val input: String = """fjkfpm (69) -> kohxzh, liwvq, eqkio, xvoyybs
dsiixv (52)
fhimhm (66)
mdlubuq (73)
ulobwyb (41)
...
fun getRoot(input: String): String {
    var doesHaveParent = hashMapOf<String, Boolean>()
    for(line in input.split("\n")){
        var elems = line.split(" ");

        var name = elems[0];
        var weight = Integer.parseInt(elems[1].trim('(', ')'));

        towers[name] = Tower(name=name, myWeight=weight);
        if(!doesHaveParent.containsKey(name)){
            doesHaveParent.put(name, false);
        }

        if(elems.size>3) {
            var subtowers = elems.subList(3, elems.size).map { it.trim(',') };
            towers[name]!!.subtowers = subtowers;
            subtowers.forEach {
                doesHaveParent.put(it, true)};
        }
    }

    return doesHaveParent.filter { !it.value }.keys.first();
}

fun getFirstUnbalancedNode(node:String) : String? {
    var sum: Int = towers[node]!!.myWeight;

    for(tower in towers[node]!!.subtowers!!) {
        var unbalancedNode = getFirstUnbalancedNode(tower);
        sum+=towers[tower]!!.totalWeight!!;
        if(unbalancedNode!=null) {
            return unbalancedNode;
        }
    }
    towers[node]!!.totalWeight = sum;
    if(towers[node]!!.subtowers.size>0) {
        var weights = towers[node]!!.subtowers!!.map { towers[it]!!.totalWeight }.filterNotNull().sorted();

        if(weights[0]!=weights.last()) {
            return node;
        }
...
data class Tower(val name: String, val myWeight: Int, var totalWeight: Int? = null, var subtowers: List<String> = emptyList());
var towers = hashMapOf<String, Tower>();
fun main(args: Array<String>) {
    var root: String = getRoot(input);
    var unbalancedNode: String = getFirstUnbalancedNode(root)!!;

    var subweights: List<Int> = towers[unbalancedNode]!!.subtowers.map { towers[it]!!.totalWeight }.filterNotNull();
    val median: Int = subweights.sorted().get(subweights.size/2);

    for(tower in towers[unbalancedNode]!!.subtowers){
        if(towers[tower]!!.totalWeight!=median) {
            System.out.print(median - towers[tower]!!.totalWeight!! +  towers[tower]!!.myWeight);
        }
    }
